...
# Define the source base directories and architecture
SOURCE_DIR_VAR="./dmf-mxl/build/Darwin-Clang-Release"
ARCHITECTURE="ARM64"
...
LOOPING_LIB="utils/gst-looping-filesrc/liblooping_filesrc.dylib"
...
function build_portable() {
    ...
    if [ "${TOOL_NAME}" == "loop-player" ]; then
        ...
        cp ~/mxl-hands-on/Portable-mxl-app/data/ARM64/run-loop-player.sh "${TARGET_DIR}/"
        cp ~/mxl-hands-on/Portable-mxl-app/data/ARM64/README-looping-filesrc "${TARGET_DIR}"
        cp ${SOURCE_DIR}/${LOOPING_LIB} "${TARGET_DIR}"
    fi

    if [ "${TOOL_NAME}" == "reader" ]; then
        cp ~/mxl-hands-on/Portable-mxl-app/data/ARM64/README-sink "${TARGET_DIR}"
    fi

    if [ "${TOOL_NAME}" == "writer" ]; then
        cp ~/mxl-hands-on/Portable-mxl-app/data/ARM64/README-testsrc "${TARGET_DIR}"
