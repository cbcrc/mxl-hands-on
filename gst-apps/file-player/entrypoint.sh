...
#   2. nmos-cpp-node           – NMOS IS-04 / IS-05 node on port 9500
#   3. Static file server      – React frontend on port 9700
#   4. FastAPI backend          – Media player API on port 9600 (foreground)
...
# 2. Start NMOS node
echo "[entrypoint] Starting nmos-cpp-node on port 9500..."
/home/nmos-cpp-node /home/node.json &

# Wait until the NMOS node answers
echo "[entrypoint] Waiting for NMOS node..."
for i in $(seq 1 30); do
    if curl -sf "http://localhost:9500/x-nmos/node/v1.3/" > /dev/null 2>&1; then
...
# 3. Serve the built React frontend on port 9700
