...
- Pipeline construction:
  ...
  5. **Output branch:** `compositor.src → identity(signal-handoffs=True) → videoconvert → capsfilter(video/x-raw,format=v210,width=W,height=H,framerate=num/den,colorimetry=bt709) → queue(leaky=2,max-size-buffers=2) → mxlsink(domain=<path>, ..., sync=false, async=false)`.
     - **`identity` PTS fix:** The compositor normalises all input timestamps to *running time* (elapsed ns since pipeline start). `mxlsink` expects TAI-domain timestamps (absolute ns from epoch, ≈ 1.748 × 10¹⁸). Without correction, grain indices are written near 0 (epoch 1970) and mxl-info-gui reports latency ≈ 56 years. The `identity` element has `signal-handoffs=True`; its Python `handoff` callback adds `pipeline.get_base_time()` to each buffer's PTS/DTS, restoring the TAI domain. The callback only runs in PLAYING (it returns early until `pipeline.get_base_time()` is valid), which is why `mxlsink` must skip preroll — see `async=false` below.
     ...
     - **`sync=false` on `mxlsink`:** Prevents `GstBaseSink` from trying to clock-synchronise the TAI PTS values (which it would interpret as a running-time offset ≈ 56 years in the future). The write thread inside `mxlsink` still waits for the correct TAI wall time before committing each grain.
     - **`async=false` on `mxlsink`:** Skips the PAUSED preroll. An async sink renders its **first** buffer during preroll — before the pipeline reaches PLAYING and before `base_time` is set — so the `identity` PTS-fix handoff returns early and that buffer reaches `mxlsink` with a raw running-time PTS. `mxlsink` latches its `mxl_pts_offset` from that first rendered buffer **once** for the whole pipeline lifetime (`render_video.rs` `get_or_insert`), baking in the near-epoch offset permanently. This only bites when a grain is already waiting at startup (i.e. the MXL source was started well before the keyer); back-to-back source/keyer starts dodge it by luck, which is what made the bug intermittent. With `async=false` the first buffer `mxlsink` latches always arrives in PLAYING with `base_time` set, so the PTS-fix handoff has already corrected it and the latch is always right. This is the complement to the `identity` fix, not a replacement for it — both are required.
  ...
  8. Set `ignore-inactive-pads=True` on the `compositor` element so it begins producing output as soon as the background branch is ready, without waiting for the CEF branch. CEF takes 1–3 s to start; without this the pipeline stalls at caps negotiation.
  9. **Set a compositor `latency` wait window.** With `latency=0` (the default) the compositor never waits for a slightly-late input buffer — on any input jitter it composites whatever frame is currently held, producing stroboscopic past/future frame flashing. This is provoked by `async=false` on `mxlsink` (above), which collapses the latency the pipeline would otherwise negotiate. Restore a wait window by setting the compositor's `latency` to a couple of frame durations, scaled from the grain rate: `frame_dur_ns = Gst.SECOND * den // num`; `compositor.set_property("latency", _COMPOSITOR_LATENCY_FRAMES * frame_dur_ns)`. `_COMPOSITOR_LATENCY_FRAMES` is a module constant (currently `2`) — higher values absorb more jitter at the cost of output latency. Wrap the `set_property` in a try/except and log a warning if the property is unavailable. (This artifact is specific to the `compositor` + leaky-queue setup here; `mxlsrc` is used unchanged in `mxl2webrtc` and `input-selector` without it.)
  10. Set `zorder` so the CEF pad sits above the background: `self._bg_pad.set_property("zorder", 0)`; `self._cef_pad.set_property("zorder", 1)`.
...
mxlsrc(input)  → videoconvert → capsfilter(BGRA) → queue(leaky) ──────────────────────────────────────────────────────────┐
                                                                                                                            ├→ compositor → identity(pts-fix) → videoconvert → capsfilter(v210, WxH, num/den, colorimetry=bt709) → queue(leaky) → mxlsink(output, sync=false, async=false)
cefsrc(url)    → capsfilter(BGRA, WxH, int-fps) → videorate → capsfilter(BGRA, num/den) → queue(leaky) ────────────────────┘
       (compositor: ignore-inactive-pads=true; latency=_COMPOSITOR_LATENCY_FRAMES frames; sink_1: sync=false, alpha=0.0 at start)
```
...
- A prose explanation covering: why `compositor` (CPU) is used instead of `glvideomixer` (GPU performance hit + memory bandwidth), why `sync=false` is required on the CEF pad of the compositor (CEF runs on the system clock, MXL runs on the live MXL clock), why `videorate` is in the CEF branch (steady frame rate to the mixer regardless of browser rendering jitter), why the leaky queues are mandatory on every branch (live pipeline — never accumulate stale frames), why `async=false` on `mxlsink` is required so its one-shot `mxl_pts_offset` latch lands on a PLAYING (PTS-fixed) buffer rather than a preroll buffer, why the compositor needs a `latency` wait window to avoid stroboscopic frame flashing once `async=false` collapses the negotiated latency, how the key is toggled live by setting `alpha` on `compositor.sink_1`, how the output format is derived from the input flow's `flow_def.json`, and the implications of CEF rendering progressive when the input is interlaced.
...
| Latency ≈ 56–67 years **intermittently** — only when the MXL source was started well before the keyer | The `identity` handoff returns early until `base_time` is set (PLAYING). As an **async** sink, `mxlsink` renders its first buffer during PAUSED preroll (before PLAYING), so that buffer skips the PTS fix and carries a raw running-time PTS. `mxlsink` latches its `mxl_pts_offset` from that one buffer for the whole pipeline life (`render_video.rs` `get_or_insert`), baking in the epoch offset. It only reproduces when a grain is already waiting at preroll, hence the source-start-timing dependency | Set `async=false` on `mxlsink` so it skips preroll; the first buffer it latches then always arrives in PLAYING with `base_time` set and already corrected by the `identity` handoff. Complements the `identity` fix — both are needed |
| Stroboscopic past/future frame flashing on the background under input jitter | `async=false` on `mxlsink` collapses the latency the pipeline would otherwise negotiate, leaving the `compositor` at its default `latency=0`. With no wait window it never waits for a slightly-late input buffer and composites whatever frame is currently held | Set the `compositor` element's `latency` to a few frame durations scaled from the grain rate (`_COMPOSITOR_LATENCY_FRAMES * Gst.SECOND * den // num`, default 2 frames). Restores a wait window for late inputs; raise `_COMPOSITOR_LATENCY_FRAMES` if flashing persists (input leaky-queue depth is the next lever) |
...
| Pipeline stalls at startup for 1–3 seconds, then reports NOT_NEGOTIATED | CEF takes time to launch its subprocess and deliver the first frame; the compositor blocks until all pads are ready | Set `ignore-inactive-pads=True` on the `compositor` element so it produces output as soon as the background branch is ready |
