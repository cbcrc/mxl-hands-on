...
- **MXL Flow Identity (output):** The single output flow UUID is **deterministic** — derived via UUID v5 from a fixed application namespace and the name `"<grouphint>:video"` (e.g. `"HTML5-Keyer:video"`). Restarting the pipeline with the same group hint reuses the same UUID and overwrites the existing flow directory in the domain. Changing the group hint produces a different UUID. The application namespace UUID is a constant defined in `gst_keyer.py` (`_MXL_KEYER_NS`) and must never change after deployment, as doing so would orphan previously written flow directories. Flow metadata is written by `mxlsink` itself: set its `group-hint` property to `"<user-grouphint>:Video"` and its `description` / `label` properties to the values provided by the user in the Setup section. The sink writes them into `flow_def.json` (including the `tags["urn:x-nmos:tag:grouphint/v1.0"]` group-hint tag) when it creates the flow — there is no post-hoc patching of the file.
...
# Pin the commit: gstcefsrc has no releases and master changes cefsrc's
# negotiation behaviour from time to time.
ARG GSTCEFSRC_COMMIT=c89c2ce5be967cfdb26920ad850436b93e62f2fc
RUN git clone https://github.com/centricular/gstcefsrc.git \
 && git -C gstcefsrc checkout --detach "${GSTCEFSRC_COMMIT}"
...
- Pipeline construction:
  ...
  4. **CEF overlay branch:** `cefsrc(url=<url>, max-video-framerate=cef_fps/1) → capsfilter(video/x-raw,format=BGRA,width=W,height=H) → videorate → capsfilter(video/x-raw,format=BGRA,framerate=num/den) → queue(leaky=2,max-size-buffers=2,max-size-time=0,max-size-bytes=0) → compositor.sink_1`. `cefsrc` renders at an **integer** framerate — compute `cef_fps = max(1, round(num/den))` and pass it as the `max-video-framerate` property (`Gst.util_set_object_arg(cef_src, "max-video-framerate", f"{cef_fps}/1")`, since the image has no PyGObject `Gst` overrides and `Gst.Fraction` is therefore unusable). The first capsfilter must **not** carry a `framerate` field: `cefsrc` advertises `framerate=0/1` (variable — one buffer per browser paint) and would fail to link. `videorate` then re-paces the output to the exact `num/den` grain rate; the second capsfilter pins that rate downstream. Without `videorate` the mixer drops/duplicates frames irregularly when the browser renders at a different cadence.
  5. **Output branch:** `compositor.src → videoconvert → capsfilter(video/x-raw,format=v210,width=W,height=H,framerate=num/den,colorimetry=bt709) → queue(leaky=2,max-size-buffers=2) → mxlsink(domain=<path>, ..., sync=false)`.
     - **No PTS fixup after the compositor:** the compositor normalises all input timestamps to *running time* (elapsed ns since pipeline start), which is what `mxlsink` expects — it maps a buffer to a grain index as `pts + base_time + (mxl_now − pipeline_clock_now)`. Do **not** re-add `pipeline.get_base_time()` in an `identity(signal-handoffs=True)` handoff: that was needed against the MXL v1.0 sink, which mapped raw PTS through `(mxl_now − clock.time())`; the v1.1 sink adds `base_time` itself, so doing it here as well double-counts it and lands every grain hours in the future.
...
- Set the output `mxlsink`'s `group-hint`, `description`, and `label` properties when the branch is built; the sink writes them into `flow_def.json` when it creates the flow.
...
cefsrc(url, max-video-framerate=int-fps) → capsfilter(BGRA, WxH) → videorate → capsfilter(BGRA, num/den) → queue(leaky) ─────────────┘
...
| `cefsrc can't handle caps … framerate=(fraction)30/1` at link time | `cefsrc` advertises `framerate=0/1` (variable) and takes its render rate from the `max-video-framerate` property instead of the caps | Drop the `framerate` field from the capsfilter immediately after `cefsrc`, compute `cef_fps = max(1, round(num/den))` and pass it as `max-video-framerate`; keep the second capsfilter after `videorate` to pin the exact `num/den` |
...
| mxl-info-gui shows the output flow hours in the future (large negative latency) | Something is adding `base_time` to the buffer PTS before `mxlsink` — e.g. a leftover `identity(signal-handoffs=True)` PTS fixup written for the MXL v1.0 sink | Remove it. Since v1.1 `mxlsink` maps a buffer as `pts + base_time + (mxl_now − pipeline_clock_now)` and wants the compositor's plain running-time PTS |
