...
| Black fill slot stalls the pipeline when selected | `videotestsrc` defaults to `is-live=false`, which does not match `mxlsrc` (live source) | Set `is-live=true` on every `videotestsrc` used as a fill slot |
