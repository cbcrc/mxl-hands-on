# GitHub Copilot Agent Instructions: NMOS-Controlled GStreamer Test Generator

## Project Overview
Your task is to build a Dockerized media application that functions as a video and audio test generator. The application uses GStreamer for media functionalities, FastAPI for the backend API, React + Vite for the web frontend, and is controllable via NMOS. This is the second application of many. The final goal is illustrated in the mermaid diagram located here `./Exercises/Exercise5.md`. You can look at the `./docker/exercise-5/data/file-player` for an example of an application, in this case, the file player. You need to keep code consistent between application
...
- **Media Format:** The test generator must support **1920x1080p60** video and **2 and 6 channels 48khz 24 bits** audio.
- **GStreamer Pipeline:** The pipeline needs to be able to dynamically change video and audio test patterns, and output it via the purposed build mxl sink that is already compiled in the project and the instruction on usage can be found at `./dmf-mxl/rust/gst-mxl-rs/readme.md`. The resulting flow_def.json should have a description and a label that identify the Test Generator application.
...
1. **Select Video Test Pattern:** A drop down list to select one of the available video test pattern in Gstreamer and start playing it. By default the app start with 100% bars selected.
2. **Select Audio Test Pattern:** A drop down list to select one of the available audio test pattern in Gstreamer and start playing it. By default the app start with 1Khz tone @ -20dbfs selected.
3. **Stereo or 6 channels selector**: A slider to select if the audio output is 2 channels or 6 channels.
4. **Audio level**: A fader to select the audio level, increment of 0,5 db, and a display to show a numerical value. The default value is -20dbfs when the app is started.
5. **Time Code option:** A tick box to select wether or not we want burn-time code. By default the application start with time code on.
6. **Ident**: A text box to enter the desired Identification overlay that will be displayed on top of the video test pattern.
...
- Create a FastAPI application on port 9610 and need to register to the registry started by `./docker/exercise-5/docker-compose.yml`.
- Implement a GStreamer wrapper class using `gi.repository.Gst`.
- Implement endpoints: `/Video-test-pattern`, `/audio-test-pattern`, `/channel-select`, `/audio-level-set`, `/audio-level-read`, `/time-code`, `/ident`.
...
- Nmos should be exposed on port 9510 for this application.
...
- the front end should be exposed on port 9710

Please write the necessary Dockerfiles, Python backend scripts, React components, and integration code following these guidelines at the following location `./docker/exercise-5/data/test-generator`.